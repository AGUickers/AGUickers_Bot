...
bot.onText(/\/delcc/, (msg, match) => {
   ...
   if (adminCheck(msg.from.id) == false) return;
   var keyboard = [];
   var custom_commands = settings.prepare(`SELECT * FROM custom_commands_${getLocale(msg.from.id, defaultlang)}`).all();
   ...
   bot.once("callback_query", (callback) => {
      switch (callback.data) {
         case "cancel":
            return bot.sendMessage(chatId, messages.messages.cancelled);
         default:
            var cmd = settings.prepare(`SELECT * FROM custom_commands_${getLocale(msg.from.id, defaultlang)} WHERE string = ?`).get(callback.data);
            if (cmd.string == undefined) return;
            settings.prepare(`DELETE FROM custom_commands_${getLocale(msg.from.id, defaultlang)} WHERE string = ?`).run(callback.data);
                  ...
                  bot.once("callback_query", (callback) => {
                     switch (callback.data) {
                        ...
                        case "string":
                           ...
                           bot.once("message", (callback) => {
                              if (callback.text == "cancel") return bot.sendMessage(chatId, messages.messages.cancelled);
                              settings.prepare(`UPDATE custom_commands_${getLocale(msg.from.id, defaultlang)} SET string = ? WHERE string = ?`).run(callback.text, cmd.string);
                        ...
                        case "text":
                           ...
                           bot.once("message", (callback) => {
                              if (callback.text == "cancel") return bot.sendMessage(chatId, messages.messages.cancelled);
                              settings.prepare(`UPDATE custom_commands_${getLocale(msg.from.id, defaultlang)} SET response = ? WHERE string = ?`).run(callback.text, cmd.string);
                              return bot.sendMessage(chatId, messages.messages.cc_edited);
                           });
                           break;
                        default:
                     ...
                     switch (callback.data) {
                        ...
                        case "link":
                           ...
                           bot.once("message", (callback) => {
                              ...
                              settings.prepare(`UPDATE custom_commands_${getLocale(msg.from.id, defaultlang)} SET link = ? WHERE string = ?`).run(callback.text, cmd.string);
                              return bot.sendMessage(chatId, messages.messages.cc_edited);
                           });
                        default:
                           break;
